...
<template>
    <button @click="modalOpen = true" class="flex items-center gap-2 bg-green-400 hover:bg-green-300 rounded px-3 py-2">
    ...
    <div v-if="modalOpen" class="h-screen fixed bg-black z-[998] w-full left-0 
        ...
        <div v-if="modalOpen" class="gap-3 p-4 flex flex-col bg-white rounded fixed z-[999] w-[30rem] 
            ...
            <form @submit="createList({listName, board_id: board_id})">
                <input type="text"  v-model="listName" id="create_list_form" class="mb-3 w-[28rem] px-2 text-lg py-2 rounded border border-black" />
